...
public class LogExportCronTask extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		String bigqueryFieldExporterSetBag = req.getParameter(AnalysisConstants.BIGQUERY_FIELD_EXPORTER_SET_BAG_PARAM);
		if ( !AnalysisUtility.areParametersValid(bigqueryFieldExporterSetBag))
		{
			bigqueryFieldExporterSetBag = this.getDefaultBigqueryFieldExporterSetBag();
		}
		BigqueryFieldExporterSetBag exporterSetBag = AnalysisUtility.instantiateExporterSetBag(bigqueryFieldExporterSetBag);
		
		// If there is no bigqueryFieldExporterSetBag, we have to add default ExporterSet List to ExporterSetBag 
		if (exporterSetBag.getExporterSetList() == null || exporterSetBag.getExporterSetList().size() <= 0 )
		{
			exporterSetBag.setExporterSetList(
					Arrays.asList(
							AnalysisUtility.instantiateExporterSet(getDefaultBigqueryFieldExporterSet())
					)
			);
		}
...
//					AnalysisUtility.instantiateExporterSet(bigqueryFieldExporterSet);
			...
			AnalysisUtility.fetchCloudStorageLogUris(
					bucketName, String.format("%s_%s", exporterSet.getPrefix(), schemaHash), startMs, endMs, requestFactory, urisToProcess, true);
...
//				tableName = tableName.replace(":", "").replace("-", "").replace(".", "");
				...
				TaskOptions taskOptions = Builder
						...
						.param(AnalysisConstants.BIGQUERY_FIELD_EXPORTER_SET_PARAM, exporterSet.getClass().getName())
//						.param(AnalysisConstants.BIGQUERY_FIELD_EXPORTER_SET_BAG_PARAM, bigqueryFieldExporterSetBag)
